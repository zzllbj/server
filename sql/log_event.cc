                                   ...
                                   PRINT_EVENT_INFO *print_event_info)
{
  ...
  {
    my_b_printf(file, "### Row event for unknown table #%llu",
                (ulong) m_table_id);
    return;
  }
...
Rows_log_event::Rows_log_event(THD *thd_arg, TABLE *tbl_arg, ulonglong tid,
...
int Rows_log_event::do_apply_event(rpl_group_info *rgi)
{
  ...
  */
  ...
  DBUG_PRINT("debug", ("m_table: 0x%lx, m_table_id: %llu%s",
                                ...
                                const char *delimiter)
{
           ...
           opt_binlog_rows_event_max_encoded_size)
  {
    ...
    */
    ...
    if (my_b_copy_to_file(body, file, (uint32) cache_size/2 + 1))
    {
      body->error= -1;
      goto end;
    }
    ...
    if (my_b_copy_to_file(body, file, (uint32) SIZE_T_MAX))
    {
      body->error= -1;
      goto end;
    }
