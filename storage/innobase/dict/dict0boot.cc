...
/** Get the encrypt status of all tablespace from dictionary header page.
@return the value as ALL_ENCRYPTED, ALL_DECRYPTED and MIXED_STATE. */
...
void dict_hdr_set_crypt_status(ib_uint32_t status)
{
	...
	mtr_start(&mtr);
	dict_hdr = dict_hdr_get(&mtr);
	mlog_write_ulint(dict_hdr + DICT_HDR_CRYPT_STATUS, status, MLOG_4BYTES,
			 &mtr);
	mtr_commit(&mtr);
}
